package main

import (
	"context"
	"encoding/csv"
	"flag"
	"fmt"
	"log"
	"math"
	"os"
	"time"

	"github.com/jackc/pgx/v5/pgxpool"
	"github.com/joho/godotenv"
)

func main() {
	// Флаги
	outputDir := flag.String("output-dir", "/Users/viktor/Desktop", "Директория для выходного CSV файла")
	limit := flag.Int("limit", 0, "Ограничение количества строк для тестирования (0 = без ограничений)")
	flag.Parse()

	_ = godotenv.Load()

	pool, err := pgxpool.New(context.Background(), os.Getenv("DB_DSN"))
	if err != nil {
		log.Fatal(err)
	}
	defer pool.Close()

	ctx := context.Background()

	fmt.Println("================================================================")
	fmt.Println("Выгрузка цен и остатков для каталога дисков")
	fmt.Println("================================================================")
	fmt.Println()

	// Генерируем имя файла с текущей датой
	dateStr := time.Now().Format("20060102") // YYYYMMDD
	filename := fmt.Sprintf("Переоценка_диски_%s.csv", dateStr)
	if *limit > 0 {
		filename = fmt.Sprintf("Переоценка_диски_%s_TEST_%d.csv", dateStr, *limit)
	}

	fmt.Printf("📦 Экспорт цен и остатков\n")
	if *limit > 0 {
		fmt.Printf("⚠️  ТЕСТОВЫЙ РЕЖИМ: ограничение %d строк\n", *limit)
	}
	fmt.Println()

	exported := exportPrices(ctx, pool, *outputDir, filename, *limit)

	fmt.Println()
	fmt.Println("================================================================")
	fmt.Printf("✅ Экспортировано: %d позиций\n", exported)
	fmt.Printf("📁 Файл: %s/%s\n", *outputDir, filename)
	fmt.Println()
	fmt.Println("✅ Файл готов для импорта в 1C-Битрикс!")
	fmt.Println("================================================================")
}

func exportPrices(ctx context.Context, pool *pgxpool.Pool, outputDir, filename string, limit int) int {
	// SQL запрос для получения цен и остатков ДИСКОВ
	limitClause := ""
	if limit > 0 {
		limitClause = fmt.Sprintf("LIMIT %d", limit)
	}

	query := fmt.Sprintf(`
		WITH filtered_stock AS (
			SELECT
				p.cae,
				p.provider,
				p.stock,
				p.price,
				p.isimport
			FROM rims_prices_stock p
			INNER JOIN nomenclature_rims n ON p.cae = n.cae
			WHERE p.stock > 0
		),
		grouped_stock AS (
			SELECT
				cae,
				SUM(stock) as total_stock,
				MIN(price) as min_price,
				MIN(isimport) as has_isimport_0,
				-- Проверяем наличие быстрых поставщиков (case-insensitive)
				SUM(CASE
					WHEN LOWER(provider) IN ('запаска', 'форточки', 'бигмашин')
					THEN 1
					ELSE 0
				END) as has_fast_provider
			FROM filtered_stock
			GROUP BY cae
			HAVING
				SUM(stock) > 4
				AND MIN(isimport) = 0
		)
		SELECT
			cae,
			total_stock,
			min_price,
			has_fast_provider
		FROM grouped_stock
		ORDER BY cae
		%s
	`, limitClause)

	rows, err := pool.Query(ctx, query)
	if err != nil {
		log.Fatalf("Query failed: %v", err)
	}
	defer rows.Close()

	// Создаем CSV
	filepath := fmt.Sprintf("%s/%s", outputDir, filename)
	file, err := os.Create(filepath)
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()

	writer := csv.NewWriter(file)
	writer.Comma = ',' // Используем запятую как разделитель
	defer writer.Flush()

	// Заголовок (4 колонки)
	header := []string{
		"IE_XML_ID", "CP_QUANTITY", "CV_PRICE_1", "CV_CURRENCY_1",
	}

	if err := writer.Write(header); err != nil {
		log.Fatal(err)
	}

	fmt.Println("✅ Логика экспорта:")
	fmt.Println("  ✓ Источник: rims_prices_stock + nomenclature_rims")
	fmt.Println("  ✓ Фильтр: SUM(stock) > 4 AND MIN(isimport) = 0")
	fmt.Println("  ✓ CP_QUANTITY: SUM(stock) по CAE")
	fmt.Println("  ✓ CV_PRICE_1: CEIL(MIN(price) * 1.12) - округление вверх")
	fmt.Println()
	fmt.Println("Экспорт данных...")

	count := 0
	exportedCAEs := []string{}

	for rows.Next() {
		var (
			cae                      string
			totalStock, hasFastProvider int
			minPrice                 int
		)

		if err := rows.Scan(&cae, &totalStock, &minPrice, &hasFastProvider); err != nil {
			log.Fatal(err)
		}

		exportedCAEs = append(exportedCAEs, cae)

		record := make([]string, 4)

		// 1. IE_XML_ID (CAE)
		record[0] = cae

		// 2. CP_QUANTITY - суммарный остаток
		record[1] = fmt.Sprintf("%d", totalStock)

		// 3. CV_PRICE_1 - минимальная цена + 12%, округление вверх
		priceRub := float64(minPrice)
		priceWithMarkup := priceRub * 1.12
		// Округление вверх до целого числа
		priceRounded := int(math.Ceil(priceWithMarkup))
		record[2] = fmt.Sprintf("%d", priceRounded)

		// 4. CV_CURRENCY_1 - валюта
		record[3] = "RUB"

		if err := writer.Write(record); err != nil {
			log.Fatal(err)
		}

		count++
	}

	// После экспорта - обновить isimport = 1 для всех экспортированных товаров
	// В тестовом режиме (limit > 0) НЕ обновляем isimport
	if len(exportedCAEs) > 0 && limit == 0 {
		fmt.Println()
		fmt.Printf("📝 Обновление isimport для %d товаров...\n", len(exportedCAEs))

		updateQuery := `
			UPDATE rims_prices_stock
			SET isimport = 1
			WHERE cae = ANY($1) AND isimport = 0
		`

		result, err := pool.Exec(ctx, updateQuery, exportedCAEs)
		if err != nil {
			log.Printf("⚠️  Ошибка обновления isimport: %v", err)
		} else {
			rowsAffected := result.RowsAffected()
			fmt.Printf("✅ Обновлено %d строк (isimport = 1)\n", rowsAffected)
		}
	} else if limit > 0 {
		fmt.Println()
		fmt.Println("⚠️  ТЕСТОВЫЙ РЕЖИМ: isimport НЕ обновлен")
	}

	return count
}

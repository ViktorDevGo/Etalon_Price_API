...
func main() {
	// Флаги
	output := flag.String("output", "/Users/viktor/Desktop/bitrix_catalog_fixed_names.csv", "Путь к выходному CSV")
	...
	fmt.Println("================================================================")
	fmt.Println("Экспорт для 1C-Битрикс (ИСПРАВЛЕННЫЕ НАЗВАНИЯ)")
	fmt.Println("================================================================")
	fmt.Println()
	...
	for rows.Next() {
		...
		// 66. CP_WEIGHT (index 65)
		record[65] = "0"

		// 72. CV_PRICE_1 (index 71)
		priceRub := float64(priceKopeks) / 100.0
		record[71] = fmt.Sprintf("%.2f", priceRub)

		// 73. CV_CURRENCY_1 (index 72)
		record[72] = "RUB"
	...
	fmt.Printf("📁 Файл: %s\n", *output)
	fmt.Println()
	fmt.Println("📋 Примеры исправлений:")
	fmt.Println("  До:  Yokohama Geolandar G91AT 225/65 R1700 102H")
	fmt.Println("  После: Yokohama Geolandar G91AT 225/65 R17 102H")
	fmt.Println()
	fmt.Println("  До:  Michelin X Line Energy F 385/65 R2250C 160K")
	fmt.Println("  После: Michelin X Line Energy F 385/65 R22.5C 160K")
	fmt.Println("================================================================")
}

...
func main() {
	// Флаги
	output := flag.String("output", "/Users/viktor/Desktop/bitrix_catalog_final.csv", "Путь к выходному CSV")
	...
	// Заголовок (75 колонок - добавлены CATALOG_QUANTITY и CATALOG_STORE_AMOUNT_1)
	header := []string{
		...
		"CATALOG_QUANTITY",        // index 62 ← НОВАЯ КОЛОНКА 1
		"CP_QUANTITY_TRACE",       // index 63
		"CP_AVAILABLE",            // index 64
		"CP_CATALOG_TYPE",         // index 65
		"CATALOG_STORE_AMOUNT_1",  // index 66 ← НОВАЯ КОЛОНКА 2
	...
	fmt.Println("  ✓ Оба поля дублируют значение CP_QUANTITY")
	...
	for rows.Next() {
		...
		record := make([]string, 75) // Увеличено с 74 до 75
		...
		// 63. CATALOG_QUANTITY (index 62) ← НОВОЕ ПОЛЕ 1
		...
		// 67. CATALOG_STORE_AMOUNT_1 (index 66) ← НОВОЕ ПОЛЕ 2
		record[66] = strconv.Itoa(totalStock)

		// 68. CP_WEIGHT (index 67) ← сдвинуто с 66
		record[67] = "0"

		// 74. CV_PRICE_1 (index 73) ← сдвинуто с 72
		priceRub := float64(priceKopeks) / 100.0
		record[73] = fmt.Sprintf("%.2f", priceRub)

		// 75. CV_CURRENCY_1 (index 74) ← сдвинуто с 73
		record[74] = "RUB"

...
func main() {
	output := flag.String("output", "/Users/viktor/Desktop/bitrix_with_quantity.csv", "Путь к выходному CSV")
	...
	fmt.Println("================================================================")
	fmt.Println("Экспорт для 1C-Битрикс (с полем QUANTITY)")
	fmt.Println("================================================================")
	fmt.Println()
	...
	header := []string{
		...
		"CP_QUANTITY", "CATALOG_QUANTITY", "QUANTITY",
		"CP_QUANTITY_TRACE", "CP_AVAILABLE", "CP_CATALOG_TYPE",
		"CATALOG_STORE_AMOUNT_1", "CV_PRICE_1", "CV_CURRENCY_1",
	}
	...
	fmt.Println("✅ Добавлена колонка QUANTITY")
	fmt.Println("  Все поля количества имеют одинаковое значение:")
	fmt.Println("    • CP_QUANTITY")
	fmt.Println("    • CATALOG_QUANTITY")
	fmt.Println("    • QUANTITY")
	fmt.Println("    • CATALOG_STORE_AMOUNT_1")
	fmt.Println()
	...
	for rows.Next() {
		...
		stockStr := strconv.Itoa(totalStock)
		record[61] = stockStr
		record[62] = stockStr
		record[63] = stockStr
		record[64] = "Y"
	...
	fmt.Printf("📁 Файл: %s\n", *output)
	fmt.Println()
	fmt.Println("📋 Поля количества:")
	fmt.Println("  CP_QUANTITY | CATALOG_QUANTITY | QUANTITY | CATALOG_STORE_AMOUNT_1")
	fmt.Println("       6      |         6        |     6    |           6")
	fmt.Println()
...
func buildQuery(limit int) string {
	query := "SELECT t.cae, COALESCE(t.brand, '') as brand, COALESCE(t.model, '') as model, t.width, t.height, t.diameter, COALESCE(t.season, '') as season, CASE WHEN t.is_studded = 'шип' THEN 'Шипованные' WHEN t.is_studded = 'не шип' THEN 'Нешипованные' ELSE '' END as studded, COALESCE(t.load_index, '') as load_index, COALESCE(t.speed_index, '') as speed_index, COALESCE(t.runflat, '') as runflat, COALESCE(MIN(p.price), 0) as price_kopeks, COALESCE(SUM(p.stock), 0) as total_stock FROM nomenclature_tyres t INNER JOIN tyres_prices_stock p ON t.cae = p.cae GROUP BY t.id, t.cae, t.brand, t.model, t.width, t.height, t.diameter, t.season, t.is_studded, t.load_index, t.speed_index, t.runflat ORDER BY t.id"
	if limit > 0 {

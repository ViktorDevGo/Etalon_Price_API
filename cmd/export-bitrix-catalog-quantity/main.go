...
func main() {
	...
	fmt.Println("================================================================")
	fmt.Println("Экспорт для 1C-Битрикс (с CATALOG_QUANTITY)")
	fmt.Println("================================================================")
	fmt.Println()
	...
	// Заголовок (74 колонки - добавлена CATALOG_QUANTITY)
	header := []string{
		...
		"CATALOG_QUANTITY",        // index 62 ← НОВАЯ КОЛОНКА
	...
	fmt.Println("  ✓ CATALOG_QUANTITY добавлена после CP_QUANTITY")
	fmt.Println("  ✓ Значение дублирует CP_QUANTITY")
	...
	for rows.Next() {
		...
		// 63. CATALOG_QUANTITY (index 62) ← НОВОЕ ПОЛЕ
		record[62] = strconv.Itoa(totalStock)

		// 64. CP_QUANTITY_TRACE (index 63) ← сдвинуто с 62
		record[63] = "Y"

		// 65. CP_AVAILABLE (index 64) ← сдвинуто с 63
		record[64] = "Y"

		// 66. CP_CATALOG_TYPE (index 65) ← сдвинуто с 64
		record[65] = "product"
	...
	fmt.Println("  CP_QUANTITY | CATALOG_QUANTITY | CP_QUANTITY_TRACE | ... | CV_PRICE_1 | CV_CURRENCY_1")
	fmt.Println("  6           | 6                | Y                 | ... | 142.20     | RUB")
	fmt.Println()

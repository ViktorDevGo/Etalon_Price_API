...
func main() {
	// Флаги
	output := flag.String("output", "/Users/viktor/Desktop/bitrix_catalog_corrected.csv", "Путь к выходному CSV")
	...
	header := []string{
		...
		// 62-67: Параметры товара
		"CP_QUANTITY",        // 62 (index 61)
		"CP_QUANTITY_TRACE",  // 63 (index 62)
		"CP_WEIGHT",          // 64 (index 63)
		"CP_WIDTH",           // 65 (index 64)
		"CP_HEIGHT",          // 66 (index 65)
		"CP_LENGTH",          // 67 (index 66)
		// 68-71: Цена
		"CV_QUANTITY_FROM",   // 68 (index 67)
		"CV_QUANTITY_TO",     // 69 (index 68)
		"CV_PRICE_1",         // 70 (index 69)
		"CV_CURRENCY_1",      // 71 (index 70)
	}
	...
	fmt.Println("  - CP_QUANTITY_TRACE: Y")
	...
	for rows.Next() {
		...
		// ВАЖНО: Создаем массив ТОЧНО на 71 элемент (indices 0-70)
		record := make([]string, 71)
		...
		// 64. CP_WEIGHT - вес (index 63)
		record[63] = "0"

		// 70. CV_PRICE_1 - цена (index 69) ← ЧИСЛО
		priceRub := float64(priceKopeks) / 100.0
		record[69] = fmt.Sprintf("%.2f", priceRub)

		// 71. CV_CURRENCY_1 - валюта (index 70) ← RUB
		record[70] = "RUB"
	...
	fmt.Println("  Остаток,Управление,Цена,Валюта")
	fmt.Printf("  6,Y,142.20,RUB\n")
	fmt.Println("================================================================")
}

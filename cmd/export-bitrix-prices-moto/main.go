...
func main() {
	...
	fmt.Println("================================================================")
	fmt.Println("Выгрузка цен и остатков для каталога мотошин")
	fmt.Println("================================================================")
	fmt.Println()

	// Генерируем имя файла с текущей датой
	dateStr := time.Now().Format("20060102") // YYYYMMDD
	filename := fmt.Sprintf("Переоценка_мотошины_%s.csv", dateStr)
	if *limit > 0 {
		filename = fmt.Sprintf("Переоценка_мотошины_%s_TEST_%d.csv", dateStr, *limit)
	}
...
func exportPrices(ctx context.Context, pool *pgxpool.Pool, outputDir, filename string, limit int) int {
	// SQL запрос для получения цен и остатков МОТОШИН
	limitClause := ""
	if limit > 0 {
	...
	query := fmt.Sprintf(`
		WITH filtered_stock AS (
			...
			WHERE p.stock > 0 AND n.tiretype = 'Мотошина'
		),
		grouped_stock AS (
			SELECT
				cae,

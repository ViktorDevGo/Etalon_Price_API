...
func main() {
	// Флаги
	output := flag.String("output", "/Users/viktor/Desktop/bitrix_catalog_warehouse.csv", "Путь к выходному CSV")
	...
	fmt.Println("================================================================")
	fmt.Println("Экспорт для 1C-Битрикс (СКЛАДСКОЙ УЧЁТ)")
	fmt.Println("================================================================")
	fmt.Println()
	...
	// Заголовок (74 колонки - добавлена CATALOG_STORE_AMOUNT_1)
	header := []string{
		...
		"CP_QUANTITY", "CP_QUANTITY_TRACE", "CP_AVAILABLE", "CP_CATALOG_TYPE",
		"CATALOG_STORE_AMOUNT_1", // ← НОВАЯ КОЛОНКА
	...
	fmt.Println("✅ Добавлена колонка CATALOG_STORE_AMOUNT_1")
	fmt.Println("  ✓ Диаметры: R1700 → R17, R1600 → R16, R2250C → R22.5C")
	fmt.Println("  ✓ Название: Бренд Модель Размер Индексы")
	fmt.Println("  ✓ Остаток на складе ID=1: значение из CP_QUANTITY")
	fmt.Println()
	...
	for rows.Next() {
		...
		record := make([]string, 74) // Увеличено с 73 до 74

		// 1. IE_XML_ID (index 0)
		record[0] = cae

		// 2. IE_NAME (index 1) - ИСПРАВЛЕННОЕ название
		...
		// 9. IP_PROP142 - диаметр (index 8) - ИСПРАВЛЕНО
		if diameter != nil {
		...
		// 63. CP_QUANTITY_TRACE (index 62)
		record[62] = "Y"

		// 64. CP_AVAILABLE (index 63)
		record[63] = "Y"

		// 65. CP_CATALOG_TYPE (index 64)
		record[64] = "product"

		// 66. CATALOG_STORE_AMOUNT_1 (index 65) ← НОВОЕ ПОЛЕ
		record[65] = strconv.Itoa(totalStock)

		// 67. CP_WEIGHT (index 66) ← сдвинуто с 65
		record[66] = "0"

		// 73. CV_PRICE_1 (index 72) ← сдвинуто с 71
		priceRub := float64(priceKopeks) / 100.0
		record[72] = fmt.Sprintf("%.2f", priceRub)

		// 74. CV_CURRENCY_1 (index 73) ← сдвинуто с 72
		record[73] = "RUB"
	...
	fmt.Printf("📁 Файл: %s\n", *output)
	fmt.Println()
	fmt.Println("📋 Пример строки:")
	fmt.Println("  CP_QUANTITY | CP_QUANTITY_TRACE | CP_AVAILABLE | CP_CATALOG_TYPE | CATALOG_STORE_AMOUNT_1 | ... | CV_PRICE_1 | CV_CURRENCY_1")
	fmt.Println("  6           | Y                 | Y            | product         | 6                      | ... | 142.20     | RUB")
	fmt.Println()
	fmt.Println("✅ Файл готов для импорта в 1C-Битрикс со складским учётом!")
	fmt.Println("================================================================")
}
